...
class fooding(base_craw):
  ...
  def get_content(self, soup):
    ...
    art_content = soup.find('div', class_="content")
    ...
    # remove all facebook element
    fb_tag = art_content.find('div', class_=re.compile('fb-like'))
    fb_tag.decompose()
    fb_tag = art_content.find('div', class_='pull-right')
    fb_tag.decompose()
    fb_tag = art_content.find('div', class_='clearfix')
    fb_tag.decompose()

    h_tag = art_content.find('h1').decompose()
    h_tag = art_content.find('h5').decompose()

    # remove all buttom element
    for btn_class in art_content.find_all('a', class_=re.compile('btn-default')):
      btn_class.decompose()

    #remove all a-href link
    for a_href_class in art_content.find_all(href=True):
      a_href_class.decompose()
